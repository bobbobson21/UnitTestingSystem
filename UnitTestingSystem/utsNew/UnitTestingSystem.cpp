...
void uts::ConOutputDomainsAndSubDomains(const UTSDataContainer* results, unsigned int domainIndex, unsigned int depth, bool outputRunNotices)
{
	...
	if (results->m_nodes[domainIndex].m_runNotice != nullptr)
	{
	...
	{
		ConOutputDomainsAndSubDomains(results, results->m_nodes[domainIndex].m_children[i], depth + 1, outputRunNotices);
